...
from config import MIN_CLOSED_LOOP_LENGTH_M, MAX_CLOSED_LOOP_LENGTH_M, LOOP_CLOSURE_THRESHOLD_M, \
...
class IO:
    ...
    def print_app_info():
        ...
        print("Программа объеденяет треки, сокращает количество точек с заданной точностью и удаляет лишние сегменты.")
    ...
    def input_cleaning_parameters() -> tuple[float, float, float, float]:
        ...
        simplification = input(f"Упрощение трека в метрах [по умолчанию {TRACK_SIMPLIFICATION_TOLERANCE_M}]: ").strip()
        min_length = input(f"Минимальная длина петли в метрах [по умолчанию {MIN_CLOSED_LOOP_LENGTH_M}]: ").strip()
        max_length = input(f"Максимальная длина петли в метрах [по умолчанию {MAX_CLOSED_LOOP_LENGTH_M}]: ").strip()
        threshold = input(f"Точность замыкания в метрах [по умолчанию {LOOP_CLOSURE_THRESHOLD_M}]: ").strip()

        simplification = float(simplification) if simplification else TRACK_SIMPLIFICATION_TOLERANCE_M
        min_length = float(min_length) if min_length else MIN_CLOSED_LOOP_LENGTH_M
        max_length = float(max_length) if max_length else MAX_CLOSED_LOOP_LENGTH_M
        threshold = float(threshold) if threshold else LOOP_CLOSURE_THRESHOLD_M

        return simplification, min_length, max_length, threshold
